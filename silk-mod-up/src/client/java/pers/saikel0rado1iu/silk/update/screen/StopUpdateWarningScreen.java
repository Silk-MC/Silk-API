...
/**
 * <h2 style="color:FFC800">停止更新警告屏幕</h2>
 * 停止更新警告界面
 *
...
public class StopUpdateWarningScreen extends UpdateScreen {
	 ...
	 */
	public StopUpdateWarningScreen(@Nullable Screen parent, UpdateData updateData, ClientUpdateManager updateManager, boolean canTrust) {
		super(parent, updateData, updateManager, Text.translatable(UpdateState.STOP_UPDATE.title(), updateData.modData().i18nName()));
	...
	protected void initWidgets(GridWidget gridWidget) {
		...
		MultilineTextWidget messageText = new MultilineTextWidget(0, 0,
				Text.translatable(UpdateState.STOP_UPDATE.text(), updateData.modData().i18nName()), textRenderer)
				.setMaxWidth(screenWidth - INTERVAL * 2);
		messageText.setPosition((width - messageText.getWidth()) / 2, (height - messageText.getHeight()) / 2 + 30);
