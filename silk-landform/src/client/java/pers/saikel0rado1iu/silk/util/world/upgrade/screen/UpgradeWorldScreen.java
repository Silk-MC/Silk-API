...
public class UpgradeWorldScreen extends BaseScreen {
	...
	public void render(DrawContext context, int mouseX, int mouseY, float delta) {
		...
		if (upgrader.getTotalChunkCount() > 0) {
			...
			context.drawCenteredTextWithShadow(textRenderer, Text.translatable("optimizeWorld.progress.counter", upgrader.getDeletedChunkCount() + upgrader.getHandledChunkCount(),
					upgrader.getTotalChunkCount()), width / 2, widgetStartY + 2 * textRenderer.fontHeight + 2, 0xA0A0A0);
			context.drawCenteredTextWithShadow(textRenderer, Text.translatable("optimizeWorld.progress.percentage", MathHelper.floor(upgrader.getProgress() * 100.0f)),
