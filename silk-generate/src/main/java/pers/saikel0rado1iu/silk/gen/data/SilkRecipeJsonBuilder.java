...
/**
 * <h2 style="color:FFC800">用于提供模组中常用但未提供更方便方法的配方生成方法</h2>
 *
...
public interface SilkRecipeJsonBuilder {
	@SilkApi
	static void generateEquipRecipe(RecipeExporter exporter, Ingredient ingredient, EquipFamily family) {
	...
	@SilkApi
	static void offerSmithingIngredient(RecipeExporter exporter, Ingredient base, Ingredient addition, RecipeCategory category, Item result) {
		Set<Item> items = Sets.newHashSetWithExpectedSize(2);
	...
	@SilkApi
	static void offerSmeltingInOneJson(RecipeExporter exporter, List<ItemConvertible> inputs, RecipeCategory category, ItemConvertible output, float experience, int cookingTime, String group) {
	...
	@SilkApi
	static void offerBlastingInOneJson(RecipeExporter exporter, List<ItemConvertible> inputs, RecipeCategory category, ItemConvertible output, float experience, int cookingTime, String group) {
