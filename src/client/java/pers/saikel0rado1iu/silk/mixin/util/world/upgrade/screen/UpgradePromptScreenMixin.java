...
abstract class UpgradePromptScreenMixin extends Screen {
	...
	private Widget init(Widget widget) throws IOException {
		...
		if (!(levelSummary instanceof UpgradeLevelSummary upgradeLevelSummary)) return widget;
		Text title;
		...
		return ButtonWidget.builder(title, button -> Objects.requireNonNull(client).setScreen(new UpgradePromptScreen(this, title, upgradeLevelSummary, callback, storageSession))).width(200).build();
	}
}
