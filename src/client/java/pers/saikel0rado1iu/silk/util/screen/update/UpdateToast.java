...
public abstract class UpdateToast implements Toast {
	...
	public UpdateToast(Text title, UpdateData data, Text message) {
		this.title = title;
		this.data = data;
	...
	public Visibility draw(DrawContext context, ToastManager manager, long startTime) {
		...
		RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
		context.drawTexture(data.getMod().getIcon().orElse(Silk.DATA.getIcon().orElseThrow()), -30 + 6, 4, 0, 0, 24, 24, 24, 24);
		if (TickUtil.getTime(time, TickUtil.Type.NATURAL, TimeUnit.MINUTES) < 0.5)
			context.drawText(manager.getClient().textRenderer, title, (getWidth() - manager.getClient().textRenderer.getWidth(title)) / 2, (getHeight() - manager.getClient().textRenderer.fontHeight) / 2, data.getMod().getThemeColor(), false);
		else
		...
		return TickUtil.getTime(time, TickUtil.Type.NATURAL, TimeUnit.MINUTES) < 1 ? Visibility.SHOW : Visibility.HIDE;
	}
}
