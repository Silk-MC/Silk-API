...
interface BlockEntityTypeRegistrationProvider extends MainRegistrationProvider<BlockEntityType<?>> {
	String SERVER_REGISTRAR = "pers.saikel0rado1iu.silk.spinningjenny.BlockEntityTypeRegistrationProvider.MainRegistrar";
	...
	final class MainRegistrar<T extends BlockEntityType<?>> extends MainRegistrationProvider.Registrar<T, MainRegistrar<T>> {
		...
		public T register(ModPass modPass, String id) {
			Registry.register(Registries.BLOCK_ENTITY_TYPE, modPass.modData().ofId(id), type);
